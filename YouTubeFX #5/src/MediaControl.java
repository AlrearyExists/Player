// Переменная duration нужна для работы слайдера. duration пришлось пересчитывать updateValues(), который вызывается чаще чем по setOnReady, похоже, что setOnReady, где duration должна бы переписываться никогда не вызывается
// Есть баг. Если убрать галку Loop и дослушать песню до конца, а потом передвинуть слайдер в середину, воcпроизведение запустится, но значёк плей не изменится на паузу, если пару раз клоцнуть по кнопке, воспроизведение вообще возвращается в начало
// ораклы отказались компилить это в джар и сказали писать самим
